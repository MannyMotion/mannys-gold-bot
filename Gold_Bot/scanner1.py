...
# ═══════════════════════════════════════════
# PER-PAIR BIAS HOURS (FOR DISPLAY ONLY)
# ─────────────────────────────────────────────
# On the 4H chart all candles open at 00:00, 04:00, 08:00... UTC
#
# CANDLE 1 always opens at 00:00 UTC. It CONTAINS:
#   GMT: 1AM UTC (Gold) / 2AM UTC (3AM pairs)
#   BST: 2AM BST=1AM UTC (Gold) / 3AM BST=2AM UTC (3AM pairs)
#
# CANDLE 2 always opens at 04:00 UTC. It CONTAINS:
#   GMT: 5AM UTC (Gold) / 6AM UTC (3AM pairs)
#   BST: 6AM BST=5AM UTC (Gold) / 7AM BST=6AM UTC (3AM pairs)
#
# KEY INSIGHT: DST shifts which UTC time falls inside the candle,
# but the 4H candle open hours (00:00 and 04:00 UTC) never change.
# So bias detection works on candle OPEN HOURS, not bias times.
# DST is only needed for display labels.
# ═══════════════════════════════════════════
BIAS_CANDLE1_HOUR = 0   # 00:00 UTC open — candle 1 for all pairs all seasons
BIAS_CANDLE2_HOUR = 4   # 04:00 UTC open — candle 2 for all pairs all seasons

def get_pair_bias_hours(symbol):
    """Returns (h1_utc, h2_utc) target bias times — used for display/labels only."""
    summer = is_bst()
    if symbol in THREE_AM_PAIRS:
        # 3AM BST / 2AM GMT  +  7AM BST / 6AM GMT
        return (2, 6) if not summer else (2, 6)
    # Gold: 2AM BST / 1AM GMT  +  6AM BST / 5AM GMT
    return (1, 5) if not summer else (1, 5)

def get_bias_hours():
    """Legacy — returns Gold default."""
    return 1, 5
...
# WEEKLY BIAS — PURE 4H
# ─────────────────────────────────────────────
# Candle 1: 4H bar opening at 00:00 UTC on Monday (all pairs, all seasons)
# Candle 2: 4H bar opening at 04:00 UTC on Monday (all pairs, all seasons)
# Range: wb_high = MAX(c1_high, c2_high) | wb_low = MIN(c1_low, c2_low)
# Confirmation: first subsequent 4H close above/below with ≥60% strong body
# Partial: only c1 found (before 04:00 UTC Monday)
# Auto-resets each new ISO week
# ═══════════════════════════════════════════
def get_weekly_bias(df_4h, symbol="XAUUSD"):
    df = df_4h.copy()
    ...
    if len(c2_rows) == 0:
        # Only candle 1 formed — partial (before 04:00 UTC Monday)
        wb_high    = c1['high']
        wb_low     = c1['low']
        wk_partial = True
        wk_full    = False
    ...
    # ── Confirmation: first 4H candle AFTER the bias candles with strong body break ──
    wk_bull = False
    wk_bear = False
...
# DAILY BIAS — PURE 4H
# ─────────────────────────────────────────────
# Candle 1: 4H bar opening at 00:00 UTC each day (all pairs, all seasons)
# db_high = c1 high | db_low = c1 low
# Confirmation: first 4H candle AFTER 00:00 that closes above/below
#               with ≥60% strong body
# Resets each UTC day at midnight
# ═══════════════════════════════════════════
def get_daily_bias(df_4h, symbol="XAUUSD"):
    df = df_4h.copy()
    ...
    # Sanity check — if range suspiciously tiny, use first 2 candles of day
    price_scale = db_high if db_high > 1 else 1
    if (db_high - db_low) < price_scale * 0.0003:
        first2  = today_candles.head(2)
        db_high = first2['high'].max()
        db_low  = first2['low'].min()

    # ── Confirmation: 4H candles AFTER 00:00 today with strong body break ──
...
def check_signal(last_signal_bar, last_bull_bar, last_bear_bar,
    ...
    df_1h   = get_candles(TIMEFRAME_1H, 500, sym)   # used for HTF EMA only

    if df_main is None or df_4h is None or df_1h is None:
        print(f"  ⚠ No data: {sym} {tf_name}")
        return last_signal_bar, last_bull_bar, last_bear_bar

    # Need at least 3 days of 4H bars (18 candles) for bias detection
    ...
    # ── BIAS — pure 4H candle detection, auto DST, all pairs ──
    ...
    pair_lbl   = f"4H C1(00:00UTC) + C2(04:00UTC) | Bias:{bias_h1}AM+{bias_h2}AM UTC"
    wk_status  = ("BULL" if wk_bull else "BEAR" if wk_bear else
                  f"PARTIAL(C1 only)" if wk_partial else "UNCONFIRMED")
...
def run():
    ...
    print("  ⚡ Pure 4H bias detection — candle 1 (00:00 UTC) + candle 2 (04:00 UTC)")
    print("  🔄 Auto DST — same 4H candles work in GMT and BST, all pairs")
